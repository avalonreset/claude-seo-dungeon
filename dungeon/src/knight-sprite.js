...
function initRuntimePicker() {
  ...
  const applyRuntime = (runtime, playSound = false) => {
    ...
    if (dangerButton) dangerButton.hidden = selectedRuntime !== 'codex';
    window.dispatchEvent(new CustomEvent('seo-dungeon-runtime-change', { detail: { runtime: selectedRuntime } }));
    if (playSound) SFX.play('menuConfirm');
