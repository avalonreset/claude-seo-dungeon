  ...
  steer(text, targetId) {
    return new Promise(async (resolve, reject) => {
      ...
      if (this.supportsSteer === false) {
        return reject(new Error('This Legends SEO Dungeon bridge does not support live steering. Restart Legends SEO Dungeon from the latest release and try again.'));
      }
      const id = ++this.requestId;
      this.handlers.set(id, { resolve, reject });
      this.ws.send(JSON.stringify({
