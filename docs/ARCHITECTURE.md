...
Legends SEO Dungeon packages a Codex-powered browser game around a modular,
portable SEO skill engine.

## Directory Structure

The package ships 25 sub-skills (21 core + 1 orchestrator + 1 framework
integration + 2 extension mirrors), 18 portable Markdown sub-agents, and 23
Codex TOML profiles.
...
| **Ahrefs** | Ahrefs API token | Backlink metrics, referring domains, organic keywords, content explorer data |
| **Bing Webmaster** | Bing Webmaster / IndexNow credentials | Bing link data and IndexNow submission workflows |
| **Profound** | Profound API token | LLM brand-citation tracking and alerting |
| **SE Ranking** | SE Ranking API key | AI visibility, SERP, backlink, and competitor data |
| **Unlighthouse** | Local Unlighthouse runner | Multi-page Lighthouse audits |

### Extension Convention

1. Self-contained in `extensions/<name>/`
2. Own `install.sh` (and `install.ps1` where Windows is supported) that copy files and may configure optional MCP adapters where applicable
3. Own `uninstall.sh` (and `uninstall.ps1` where present) that reverse installation
...
6. Prefer project `.env` credentials and direct scripts for normal Legends SEO Dungeon audits
