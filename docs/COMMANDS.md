...
All Legends SEO Dungeon commands start with `/seo` followed by a subcommand.

## Command List

### `/seo doctor`

Check the isolated engine without changing it: `python scripts/runtime.py doctor --json`.
Use the absolute runtime path when the current directory is a client project.

### `/seo setup`

Explicitly prepare or repair the isolated Python environment and Chromium:
`python scripts/runtime.py setup`. Report core and browser readiness separately.
...
Live SEO data via DataForSEO credentials. 23 commands across 9 API modules.
...
/seo dataforseo serp-images <keyword>       # Google Images results
...
| `/seo ahrefs [command] <target>` | Ahrefs backlink and keyword data (extension) |
| `/seo bing [command] <url>` | Bing Webmaster data and IndexNow (extension) |
| `/seo profound [command]` | LLM brand-citation tracking (extension) |
| `/seo seranking [command]` | AI share-of-voice tracking (extension) |
| `/seo unlighthouse <url>` | Multi-page Lighthouse audits (extension) |

## Release history

See [v2.2 release notes](RELEASE-v2.2.md) for the prior Dungeon baseline.
